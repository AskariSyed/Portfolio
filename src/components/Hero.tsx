...
function Hero({ isDark }: HeroProps) {
  return (
    <header id="top" className={`grid lg:grid-cols-2 gap-8 lg:gap-12 items-center p-6 sm:p-8 md:p-12 rounded-2xl sm:rounded-3xl border ${isDark ? 'bg-[#131a26]/80 border-gray-800' : 'bg-white/40 border-gray-200'} backdrop-blur transition-all duration-700 animate-fadeIn`} style={{animationDuration: '1.2s'}}>
      <div className="space-y-6">
        <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-full text-xs font-bold uppercase tracking-wide ${isDark ? 'bg-cyan-600/30 text-white' : 'bg-cyan-500/10 text-cyan-600'} animate-slideIn`} style={{animationDuration: '1.2s'}}>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-3 animate-fadeIn" style={{animationDuration: '1.5s'}}>
          <a href={`mailto:${profile.email}`} className={`inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-3 rounded-lg font-medium border transition-all text-center ${isDark ? 'border-gray-700 text-cyan-100 hover:text-cyan-400 hover:border-cyan-400/50' : 'border-slate-300 text-slate-900 hover:text-cyan-600 hover:border-cyan-600'}`}>
            <Mail size={18} /> Email
          </a>
          <a href={profile.github} target="_blank" rel="noreferrer" className={`inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-3 rounded-lg font-medium border transition-all text-center ${isDark ? 'border-gray-700 text-cyan-100 hover:text-cyan-400 hover:border-cyan-400/50' : 'border-slate-300 text-slate-900 hover:text-cyan-600 hover:border-cyan-600'}`}>
            <Github size={18} /> GitHub <ArrowUpRight size={16} />
          </a>
          <a href={cvFile} download className={`inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-3 rounded-lg font-medium border transition-all text-center ${isDark ? 'border-gray-700 text-cyan-100 hover:text-cyan-400 hover:border-cyan-400/50' : 'border-slate-300 text-slate-900 hover:text-cyan-600 hover:border-cyan-600'}`}>
            <Download size={18} /> Download CV
          </a>
          <a href="https://www.linkedin.com/in/syed-hassan-askari" target="_blank" rel="noreferrer" className={`inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-3 rounded-lg font-medium border transition-all text-center ${isDark ? 'border-gray-700 text-cyan-100 hover:text-cyan-400 hover:border-cyan-400/50' : 'border-slate-300 text-slate-900 hover:text-cyan-600 hover:border-cyan-600'}`}>
      ...
      <div className={`rounded-xl border p-4 sm:p-6 space-y-4 ${isDark ? 'bg-[#181f2e]/80 border-gray-700' : 'bg-slate-50 border-slate-200'} transition-all duration-700 animate-fadeIn`} style={{animationDuration: '1.5s'}}>
        <img src={profileImg} alt="Muhammad Hassan Askari" className="w-32 h-32 sm:w-40 sm:h-40 lg:w-44 lg:h-44 mx-auto rounded-lg border-2 border-cyan-400/30 object-cover" loading="lazy" />
